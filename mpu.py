...
Yaw has nothing to correct it - no magnetometer on a 6050 - so it drifts, a few
degrees a minute once the bias is calibrated. Re-centre in the 3D view (r) or
restart to re-zero.
"""
...
def stream(port=None, axes="x,y,z", calib_seconds=1.5, status=None):
    ...
    while True:
        ...
        if line.startswith("#"):
            ...
            for tok in line.split():
                if tok.startswith("accel_fs="):
                    ...
                    gyro_lsb = 32768.0 / float(tok.split("=")[1])
        ...
        rot = tuple((g - b) / DEG for g, b in zip(gyro_dps, bias))   # rad/s
        fil.update(rot, acc, dt)
        yaw, pitch, roll = fil.euler()
        grav = fil.gravity()
        q = fil.q

        yield Fused(t=t, yaw=yaw, pitch=pitch, roll=roll,
                    quat=(q[0], q[1], q[2], q[3]), rot=rot, acc=acc,
                    grav=grav, part=part)
